...
| Optional two-cache coherence prevents stale private copies | read sharing, write invalidation, dirty intervention, conflict eviction, and simultaneous requests | returned-data checks plus coherence event counters | single modified owner, exclusive response owner, one accepted requester | `coherence_summary.csv`, `coherence_coverage.csv`, and `coherence_and_bist.md` |
| SRAM BIST detects stuck-at faults and preserves diagnostics | clean and injected March C-minus runs at first/middle/final addresses | expected fail status, first failing address, expected/observed data | done-not-busy, stable failure metadata, BIST ownership of SRAM port | `bist_summary.csv`, `bist_coverage.csv`, and `coherence_and_bist.md` |
| Integrated cache-array BIST exercises real parity/SECDED storage | injected integrity error, destructive scan, clean repeat, post-BIST refill | expected error and first-failure metadata plus cache reuse check | failure metadata stability and maintenance release | `cache_array_bist_summary.csv`, `cache_array_bist_coverage.csv`, and `coherence_and_bist.md` |
