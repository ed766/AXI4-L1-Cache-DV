module cache_small_geometry_harness;
`ifndef CACHE_SETS
`define CACHE_SETS 2
`endif
`ifndef CACHE_WAYS
...
`endif
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      assert(!(maint_valid && maint_ready && cpu_req_valid && cpu_req_ready));
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (rst_n) begin
      ...
      cover(mon_evict);
      cover(maint_done);
      cover(cpu_rsp_valid && cpu_rsp_error);
    end
  end
endmodule
